...
* **Payment Execution:** Handled via a secure, non-custodial smart contract layer. Senders register leg payout values and approve the validator. Payer calls `registerRule` specifying Leg Recipient and amount, and must `approve` the validator for the sum of payout amounts. Payouts execute automatically once the document's signing requirements are satisfied.
...
| **Teams** (`teams`) | $35/user/mo (billed yearly) | 15 per user (pooled) | Max 10 | Everything in Solo, plus: shared templates, team envelope drafts, team envelope visibility, attached basic USDC payouts. |
| **Teams Pro** (`teams_pro`) | $59/user/mo (billed yearly) | 25 per user (pooled) | Max 15 | Everything in Teams, plus: custom integrations, seat allocation, bulk send from CSV, custom branding, webhooks, metadata tags, advanced routing, and advanced settlement rules. |
