...
| **Proof of signing** | Vendor database + PDF certificate | **`FSFileRegistry`** on Base + exportable compliance bundle |
...
Our v1 contracts are intentionally small: **`FSFileRegistry`** and **`FSPaymentValidator`**, deployed on **Base**. No upgrade proxies. Identity, key material, and sharing approvals live **off-chain** in Postgres and your browser.
...
3. **Collect signatures** on placed fields; each completion is validated and relayed to **`FSFileRegistry`**.
