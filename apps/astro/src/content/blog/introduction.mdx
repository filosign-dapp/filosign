...
Senders can attach USDC payment rules to a document so recipients are paid automatically when signing conditions are met—no custodial escrow, just an on-chain approve and a Gelato-executed transfer. This unifies signatures and payments in a single flow. Consider the following examples:
