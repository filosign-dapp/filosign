...
Senders can attach USDC settlement rules to a document so recipients are paid when signing conditions are met—no custodial escrow, just an on-chain approve and permissionless `executePayout` (Filosign server relay or your wallet). This unifies signatures and settlements in a single flow. Consider the following examples:
...
> Note: Filosign is currently in Beta and is deployed on a testnet (Base Sepolia). USDC attached to documents here is testnet USDC from faucets for experimentation; uploaded files may be wiped in future. This environment is a playground without financial risk or permanent record. Behavior matches mainnet design. A stable mainnet launch with permanent storage will follow soon.
