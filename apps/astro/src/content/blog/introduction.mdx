...
- **Verifiable signing records:** When someone signs, an attestation is relayed to **`FSFileRegistry`** on Base. Anyone with the compliance bundle and a public RPC node can verify it—no Filosign login required.
...
- **On-chain proof:** Registration and signature events are written to **`FSFileRegistry`**. Tamper-evident, publicly auditable, independent of our database.
