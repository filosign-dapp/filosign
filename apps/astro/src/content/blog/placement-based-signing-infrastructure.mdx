...
description: Placement manifests, Merkle proofs, compliance bundles, and on-chain attestations in FSEnvelopeRegistry. Evidence you can verify without trusting our servers.
readingTime: 8 min
...
interface ComplianceBundle {
  ...
  parties: Array<{
    ...
    authSubjectCommitment: `0x${string}` | null;
  }>;
  onchainRegistration: {
    ...
    senderAuthSubjectCommitment: `0x${string}`;
    signersCount: number;
...
3. **On-chain registration** (relayer pool relay, `onlyRelayer`):
...
3. **Verify the attestation:** Confirm `completionsRoot` was registered for the claimed signer in **`FSEnvelopeRegistry`**
