...
Server-side product events via [`lib/platform/analytics/analytics.ts`](lib/platform/analytics/analytics.ts) (`posthog-node`). Set `POSTHOG_HOST`, `POSTHOG_ENABLED`, and `POSTHOG_API_KEY` in `.env.local`. Full event catalog and funnel guidance: [`project/product/ops/posthog-integration.md`](../../project/product/ops/posthog-integration.md).
...
Critical platform failures emit via [`lib/platform/analytics/alerts.ts`](lib/platform/analytics/alerts.ts) using [`@filosign/logger`](../../packages/logger) (Telegram transport). Requires `TG_ANALYTICS_BOT_TOKEN` and `TG_ANALYTICS_BOT_GROUP_ID`; delivery is gated by `TG_ANALYTICS=true`. Supergroup chat IDs use a leading minus (e.g. `-1003999299780`); bare IDs copied without the minus are normalized automatically. On successful bootstrap, the server sends a one-time `server.started` info ping to the same group (api and worker each ping on their own restart). In-app product feedback (`feedback.submit`) sends a `product.feedback_submitted` info ping with wallet, feature area, rating, and message preview. New admin-actionable requests send info pings to the same group: `platform.access_request_submitted` (beta signup), `platform.payout_access_request_submitted` (settlement feature access), and `platform.partner_invite_redeemed` (partner invite redemption).

ROI ops alerts (same Telegram group, 5-minute dedupe): `server.worker_stale` (api-only monitor when worker heartbeat goes stale), `billing.webhook_stuck` (sweeper cron for aged Dodo webhook rows), `email.outbox_stuck` (outbox sweeper when rows age past 30m), `billing.subscription_problem` (Dodo `subscription.failed` / `on_hold`), `server.email_disabled` (worker startup when Resend and SES are both off outside local), and settlement `failed_insufficient` / `failed_conditions` via `settlements.relay_payout_failed`. Proactive low-balance checks (`server.relayer_gas_low`, `server.foc_fil_low`, `server.foc_usdfc_low`) run on a twice-daily cron (UTC 08:00 and 20:00).

When `POSTHOG_ENABLED=true` (with `POSTHOG_HOST` and `POSTHOG_API_KEY`), the same alert is mirrored as a sanitized `platform_alert` PostHog event with the same 5-minute dedupe as Telegram. See [`alerts.ts`](lib/platform/analytics/alerts.ts) (`mirrorPlatformAlertToPostHog`) and [`project/product/ops/posthog-integration.md`](../../project/product/ops/posthog-integration.md).

**Manual staging verification** (not run in CI): see [`project/product/ops/production-smoke-tests.md`](../../project/product/ops/production-smoke-tests.md) scenarios 7–8.
...
- **Dokploy / Docker** - image uses [`scripts/deploy/infisical-entrypoint.sh`](scripts/deploy/infisical-entrypoint.sh); set bootstrap vars per [`SECRETS.md`](SECRETS.md). Do not paste app secrets into Dokploy env UI.
- **`GET /health`** (root app, not under `/api`) - `{ ok: true }` for probes.
- **Dodo billing webhook** - `POST /api/integrations/dodo/webhook` (Standard Webhooks headers). API **acks fast**: `received` row, `invalidateOrgEntitlements`, enqueue **`billing-webhook`** queue; worker runs full sync. See [`project/product/ops/job-idempotency.md`](../../project/product/ops/job-idempotency.md).
...
- **Data lifecycle policy** - see [`project/product/ops/data-lifecycle.md`](../../project/product/ops/data-lifecycle.md) (includes PR checklist appendix).
- **Schema:** [`drizzle/`](drizzle/) - squashed baseline [`0000_initial.sql`](drizzle/0000_initial.sql); add migrations via `db:generate` only (never hand-edit SQL/journal). **Local/staging:** `push`. **Sandbox/production:** `generate` → user reviews SQL → user commits `drizzle/` (`git commit --no-verify` after review; pre-commit blocks staged drizzle on normal commit) → deploy (auto-migrate on start) or laptop tunnel. CI gate: `bun run db -- migration-check` · preview staged SQL: `bun run db -- confirm-migration-commit --staged` · smoke: `bun run db -- verify-migrations`. Logic: [`lib/platform/db/migrations/`](lib/platform/db/migrations/). See [drizzle-migrations.mdc](../../.cursor/rules/drizzle-migrations.mdc).
