...
- **Schema:** [`drizzle/`](drizzle/) - squashed baseline [`0000_initial.sql`](drizzle/0000_initial.sql); add migrations via `db:generate` only (never hand-edit SQL/journal). **Local/staging:** `push`. **Sandbox/production:** `generate` → commit → deploy (auto-migrate on start) or laptop tunnel. CI gate: `bun run db -- migration-check` · smoke: `bun run db -- verify-migrations`. Logic: [`lib/platform/db/migrations/`](lib/platform/db/migrations/). See [drizzle-migrations.mdc](../../.cursor/rules/drizzle-migrations.mdc).
