...
/** Map a Hardhat fixture to a minimal viem client bundle for EIP-712 helpers. */
export function filosignContractsFromFixture(ctx: FullSystemFixture) {
	const account = walletAccount(ctx.sender);
	return {
		FSEnvelopeRegistry: ctx.envelopeRegistry,
		FSPaymentValidator: ctx.paymentValidator,
		$client: { ...ctx.sender, account },
		$chainKey: "local" as const,
	};
}

export type FixtureContracts = ReturnType<typeof filosignContractsFromFixture>;
