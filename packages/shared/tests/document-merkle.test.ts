...
import { merkleRootFromLeafAndSiblings } from "../utils/crypto";
import {
	documentLeafHashV1,
...
describe("document Merkle v1", () => {
	...
	it("each proof verifies to the root", async () => {
		...
		for (const p of proofs) {
			const doc = documents.find((d) => d.id === p.id);
			if (!doc) throw new Error(`proof id ${p.id} not in documents`);
			const recomputed = merkleRootFromLeafAndSiblings(p.leafHash, p.siblings);
